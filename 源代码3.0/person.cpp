...
string cutebaby::reach_starting()
{
	set_money(1000);
	act_cute();
	return "";
...
int blindBoxGirl::take_blind_box()
{
	initgraph(1024, 768);
	putimage(0, 0, &choice_interface);
	char ch;
	int box_x = 0;
	int box_y = 0;
	while (1)
	...
	static default_random_engine e(time(0));
	static uniform_int_distribution<size_t> u(0, 11);
/*
	putimage(250 , 340, &box);
	putimage(500, 400, &box);
	putimage(750, 480, &box);
	Sleep(2000);
*/	
	int move_x = 0;
	for (int i = 0; i < 15; ++i)
	{
		move_x += 10;
		move_x *= (-1);
		putimage(box_x+move_x, box_y, &box);
		Sleep(100);
		putimage(0, 0, &choice_interface);
	}
	

	putimage(228, 18, &ppmt);
	
	int id = u(e);
	
	putimage(xs[id % 4]+228, ys[id / 4]+18, &arrow);
	
	this->set_money(100 + id * 100);
