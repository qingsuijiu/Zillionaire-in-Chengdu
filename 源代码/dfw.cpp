...
void dfw::operatate_estate(shared_ptr<site> current_site,int j)
{
	...
	case -2: //机会
	{
		...
		players[j]->set_money(cha.get_money_change());
        _getch();  //按任意键进入下一步
	...
	{
		//人物可能拥有的技能
		int res = players[j]->all_land_operations();
		if (res == 2)
		...
		//是否踩到该地的粑粑
		step_on_shirt(j, current_site);

		//while (_kbhit())
		//	_getch();
		//买不起的情况
		if (current_site->get_price() > players[j]->get_money())
		{
			refresh_background();
		...
		while (1)
		{
			...
			if (c == 'y')
			{
				...
				current_site->set_owner(players[j]);
				players[j]->set_money(current_site->get_price() * (-1));
				players[j]->acquire_property(current_site->get_id());

				//refresh_background();
				string temp2 = "";
				temp2 += players[j]->get_name();
				temp2 += "已购买";
	...
	case 1: //有人拥有的空地
	{
		...
		if (current_site->get_owner() == players[j])
		{
			int res = players[j]->self_land_operations();
			if (res == 1)
				go_to_prison(j);
			
			//选择建青年旅舍
			build_something(current_site, j, 1);
		}
		else
		{
			refresh_background();
			...
			temp2 += "支付过路费";
			int toll = current_site->get_price() * 0.1;
			temp2 += to_string(toll);
			temp2 += "元，来来来，给钱！";
			caption_write(1, temp2);

			players[j]->set_money((-1) * toll);
	...
	case 2:  //青年旅社
	{
		...
		if (current_site->get_owner() == players[j])
		{
			...
			//选择建宾馆
			build_something(current_site, j, 2);
		}
		else
		{
			refresh_background();
			...
			string temp2 = "";
			temp2 += players[j]->get_name();
			temp2 += "享受完青年旅舍，需向该土地拥有者";
			temp2 += current_site->get_owner()->get_name();
			temp2 += "支付过路费";
			int toll = current_site->get_price() * 0.2;
			temp2 += to_string(toll);
			temp2 += "元，来来来，给钱！";
			caption_write(1, temp2);
	...
	case 3:  //宾馆
	{
		...
		if (current_site->get_owner() == players[j])
		{
			...
			//选择建情侣酒店
			build_something(current_site, j, 3);
		}
		else
		{
			refresh_background();
			...
			string temp2 = "";
			temp2 += players[j]->get_name();
			temp2 += "享受完宾馆，需向该土地拥有者";
			temp2 += current_site->get_owner()->get_name();
			temp2 += "支付过路费";
			int toll = current_site->get_price() * 0.35;
			temp2 += to_string(toll);
			temp2 += "元，来来来，给钱！";
			caption_write(1, temp2);
	...
	case 4:  //情侣酒店
	{
		...
		if (current_site->get_owner() != players[j])
		{
			...
			string temp2 = "";
			temp2 += players[j]->get_name();
			temp2 += "享受完情侣酒店，需向该土地拥有者";
			temp2 += current_site->get_owner()->get_name();
			temp2 += "支付过路费";
			int toll = current_site->get_price() * 0.55;
			temp2 += to_string(toll);
			temp2 += "元，来来来，给钱！";
			caption_write(1, temp2);
...
void dfw::build_something(shared_ptr<site> s,int player_id,int kind)
{
	...
	while (1)
	{
		...
		if (c == 'y')
		{
			...
			s->set_status(kind + 1);
			players[player_id]->set_money(building_prices[kind - 1] * s->get_price());
			_getch();
			return;
		}
		else if (c == 'n')
